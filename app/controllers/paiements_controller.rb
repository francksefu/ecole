class PaiementsController < ApplicationController
  ...
  def create
    ...
    @student = Student.find(params[:student_id].to_i)
    @detail_paiement_classroom = DetailPaiementClassroom.find(params[:detail_paiement_classroom_id].to_i)
    @paiement = @accountant.paiements.new(student: @student, detail_paiement_classroom: @detail_paiement_classroom,
                                          montant: params[:paiement][:montant])
    if @paiement.save
      flash[:success] = 'Paiement save successfully'
      redirect_to student_classroom_year_paiements_path(@student, @detail_paiement_classroom.classroom,
                                                        @detail_paiement_classroom.detail_paiement.year)
    else
